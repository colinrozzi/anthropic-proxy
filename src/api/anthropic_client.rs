...
// Import shared types from anthropic-types
use anthropic_types::{
    AnthropicError, CompletionRequest, CompletionResponse, Message, MessageContent,
    ModelInfo, ModelPricing, Usage,
};
...
impl AnthropicClient {
    ...
    pub fn list_models(&self) -> Result<Vec<ModelInfo>, anthropic_types::AnthropicError> {
        ...
        let body = response
            .body
            .ok_or_else(|| anthropic_types::AnthropicError::InvalidResponse("No response body".to_string()))?;
    ...
    fn parse_response(&self, body: &[u8]) -> Result<CompletionResponse, anthropic_types::AnthropicError> {
        let response_data: Value = serde_json::from_slice(body)?;
        
        // Extract required fields
        let id = response_data["id"]
            .as_str()
            .ok_or_else(|| anthropic_types::AnthropicError::InvalidResponse("No message ID".to_string()))?
            .to_string();

        let model = response_data["model"]
            .as_str()
            .ok_or_else(|| anthropic_types::AnthropicError::InvalidResponse("No model info".to_string()))?
            .to_string();

        let stop_reason = response_data["stop_reason"]
            .as_str()
            .ok_or_else(|| anthropic_types::AnthropicError::InvalidResponse("No stop reason".to_string()))?
            .to_string();
            
        // Extract usage information
        let input_tokens = response_data["usage"]["input_tokens"]
            .as_u64()
            .ok_or_else(|| anthropic_types::AnthropicError::InvalidResponse("No input tokens".to_string()))?
            as u32;

        let output_tokens = response_data["usage"]["output_tokens"]
            .as_u64()
            .ok_or_else(|| anthropic_types::AnthropicError::InvalidResponse("No output tokens".to_string()))?
            as u32;
    ...
    fn parse_content_blocks(&self, content_array: &[Value]) -> Result<Vec<MessageContent>, anthropic_types::AnthropicError> {
        ...
        for block in content_array {
            ...
            match block_type {
                "text" => {
                    let text = block["text"]
                        .as_str()
                        .ok_or_else(|| anthropic_types::AnthropicError::InvalidResponse("Missing text in text block".to_string()))?
                        .to_string();
                ...
                "tool_use" => {
                    let id = block["id"]
                        .as_str()
                        .ok_or_else(|| anthropic_types::AnthropicError::InvalidResponse("Missing id in tool_use block".to_string()))?
                        .to_string();
                    
                    let name = block["name"]
                        .as_str()
                        .ok_or_else(|| anthropic_types::AnthropicError::InvalidResponse("Missing name in tool_use block".to_string()))?
                        .to_string();
                ...
                "tool_result" => {
                    let tool_use_id = block["tool_use_id"]
                        .as_str()
                        .ok_or_else(|| anthropic_types::AnthropicError::InvalidResponse("Missing tool_use_id in tool_result block".to_string()))?
                        .to_string();
